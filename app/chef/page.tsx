...
export const metadata: Metadata = {
  title: "Kitchen Display System — Chef Console",
  description: "Athidi restaurant kitchen display system for chefs.",
};
export const dynamic = "force-dynamic";

async function getChefAccess(): Promise<{ staff: StaffIdentity } | null> {
  ...
  if (!roleName || !canAccessWorkspace(roleName, "kitchen")) {
...
export default async function ChefPage() {
  const access = await getChefAccess();
  if (!access) {
    return (
      <main className="auth-page">
        <section className="auth-card">
          <h1>Backend configuration required</h1>
          <p>Add the Supabase server settings before opening the kitchen console.</p>
        </section>
      </main>
    );
  }
  return <AdminOS staff={access.staff} initialSection="Kitchen" />;
}
